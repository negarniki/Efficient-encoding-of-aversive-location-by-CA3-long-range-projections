...
cclustID.plclength =         7;     % 7 center of place field
...
cclustID.spatialInfop =     15;     % 15 spatial info p
cclustID.spatialInfo =      16;     % 16 Spatial info 
...
cclustID.cellID =           51;     % 51 logical if cell was in FOV of all experiments

save('/media/2Photon/Negar Nikbakht/ANALYSIS/cclustID','cclustID')
...
%% Cell ID
cclust(1:size(caim.C,1),cclustID.cellID) = 1:size(caim.C,1);
...
if isfield(scn,'cellID')  
    ...
    cclust(isplace,cclustID.spatialInfop) = scn.cellID(:,9);
    cclust(~isplace,cclustID.plcfld:cclustID.spatialInfop) = NaN;
    cclust(isplace,cclustID.spatialInfo) = scn.cellID(:,10);
    cclust(~isplace,cclustID.plcfld:cclustID.spatialInfo) = NaN;
    ...
    cclust(:,cclustID.spatialInfop) =  NaN; 
    cclust(:,cclustID.spatialInfo) =  NaN; 
...
if isfield(caim,'network') && ~isempty(caim.network) && isfield(caim.network,'cellID')  && ~isempty(caim.network.netID) 
