...
if length(find(diff(belt.stripesPR)))>2 && max(belt.stripesPR)>1
    stripeOfInt = 1;    
    wind = [-5 5];                                    % size of window to look for response
    thresh = 1;                                     % minimal number of detected events around stimulus
    respwind = [-1 1]; 
    stripes = belt.stripesPR;
    stripes(2:end) = diff(stripes);
    stripes(belt.stripesPR ~= stripeOfInt) = 0;
    scn.stripe1 = findstim(stripes,caim,wind,thresh,respwind,S,C); 
    if showres;disp(['Passed stripe 1 ' num2str(length(scn.stripe1.timepnt)/2) ' times']);end
end
...
if length(find(diff(belt.stripesPR)))>2 && max(belt.stripesPR)>1
    ...
    stripes(2:end) = diff(stripes);
    stripes(belt.stripesPR ~= stripeOfInt) = 0;
    scn.stripe2 = findstim(stripes,caim,wind,thresh,respwind,S,C); 
    if showres;disp(['Passed stripe 2 ' num2str(length(scn.stripe2.timepnt)/2) ' times']);end
end
...
if length(find(diff(belt.stripesPR)))>2 && max(belt.stripesPR)>1
    ...
    stripes(2:end) = diff(stripes);
    stripes(belt.stripesPR ~= stripeOfInt) = 0;
    scn.stripe3 = findstim(stripes,caim,wind,thresh,respwind,S,C); 
    if showres;disp(['Passed stripe 3 ' num2str(length(scn.stripe2.timepnt)/2) ' times']);end
end
...
%         stimplc = histcounts(round(stimpos./spcbin),1:101);
    ...
    if isfield(caim,'C')    %  extract Ca-traces for every point and every cell
        ...
        % cellID  #stim-events #stim-resps  firing frequency    firingfrequency during running  number of associated networks
        ...
        %% Output
        Resp.cellID = cellID;        
        Resp.resp = uint16(resp);
        Resp.respCa = single(respCa);
...
%         Resp.respSpec = respSpec;
        ...
        Resp.nr.resp = uint16(respnr);
        Resp.nr.respCa = single(respCanr);
...
%             pupil{j} = zscore(scn.pupil(find(scn.tsscn == timesscn(2*j-1))+wind));
    ...
    Resp.times = single(times);    
    Resp.speed = single(speed);
    Resp.running = running ==1 ;
...
%         newtimes = zeros(size(Resp.times,1),ceil(size(Resp.times,2)*FsN/Fs));
        newtimes = Resp.times(1,1):1000*(1/FsN):Resp.times(1,end); 
        for i = 1:size(Resp.times,1)
    ...
    Resp.running = double(Resp.running);
    Resp.resp = double(Resp.resp);
    Resp.nr.resp = double(Resp.nr.resp);
    ...
    for i = 1:size(Resp.times,1)
        ...
        if isfield(Resp,'pupil') && find(~isnan(Resp.pupil(i,index)),1)
        ...
        for i = 1:size(Resp.resp,2)
            ...
            if isfield(Resp,'respSpec')
                for j = 1:size(Resp.respSpec,1)
                    respSpec(j,i,:) = interp1(Resp.times(i,:),reshape(Resp.respSpec(j,i,:),size(Resp.times(i,:))),newtimes(i,:),'next');
                end
            end
            if isfield(Resp,'netresp')
                netresp = interp1(Resp.times(i,:),Resp.netresp(i,:),newtimes(i,:),'linear');
                sumnet = interp1(Resp.times(i,:),Resp.sumnet(i,:),newtimes(i,:),'linear');  
            end
        end
        if isfield(Resp,'netmean')
