...
elseif isfield(caim.bulk,'traceGC')
    disp('Cell bulk signal used')
    x = caim.bulk.traceGC(1,:);
elseif isfield(caim.bulk,'traceIn')
    disp('Over all bulk signal used')
    x = caim.bulk.traceIn(2,:);
elseif isfield(caim.bulk,'trace') && size(caim.bulk.trace,2) ~= 4 && ~isfield(caim.bulk,'traceIn') 
    disp('Over all bulk signal used and variable renamed')
    caim.bulk.traceIn = caim.bulk.trace;
    x = caim.bulk.trace(2,:);
elseif isfield(caim.bulk,'trace') && size(caim.bulk.trace,2) == 4
    disp('Over all bulk signal was previously analyzed')    
    x = caim.bulk.trace(:,1)';
else
