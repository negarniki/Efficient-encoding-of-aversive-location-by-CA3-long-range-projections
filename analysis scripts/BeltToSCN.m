...
if (isfield(belt, 'pupil') && ~isempty(find(belt.pupil,1))) && (length(isnan(belt.pupil))~=length(belt.pupil))
    %%
    thrs = .700;
    pupil = belt.pupil;
    ...
    PupOn = find(diff(c)==-1);
    if ~isempty(PupOn) && ~isempty(PupOff)
    ...
    if ~isempty(PupOff) && ~isempty(PupOn)
        ...
        for i = 1:length(PupOn)
            ...
            if length(int)>9 && sum(isinf(pupil(int))) == 0
