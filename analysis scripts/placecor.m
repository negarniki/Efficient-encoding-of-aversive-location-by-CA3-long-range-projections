...
% Output:
%
% scn.speedbin  speed values in bins on belt
% scn.waitbin   resting time in each bin on the belt
...
% scn.restcode  Events per bin that happend during rest
% scn.overround place activity averaged for each round to see development
...
%     win =  scn.rounds==i;
    ...
    for j = 1:numbin
        ...
        if ~isempty(int)
            temp = speedtemp(int);           
            if strtrnd == 0
                speedwaitbin(i+1,j) = mean(temp); 
                temp = temp(temp>0);
                speedbin(i+1,j) = mean(temp);
                waitbin(i+1,j) = sum(runningtemp(int)==0);               
            else
                speedwaitbin(i,j) = mean(temp); 
                temp = temp(temp>0);
...
%%
[spccode,spcsort,numbin,cellID,restcode] = CaOnPlace(caim,scn,spcbin,minev);

scn.spaccode = spccode;                     
scn.spcsort = spcsort;
scn.restcode = restcode;
...
%% Development of space activity over rounds

overround = sum(spccode,2);
overround = permute(overround,[1 3 2]);
...
function [spccode,spcsort,numbin,cellID,restcode] = CaOnPlace(caim,scn,spcbin,minev,cellID)
    numbin = ceil(max(scn.distance)/spcbin);
    spccode = zeros(length(caim.Df),numbin,max(scn.rounds));
    strtrnd = double(scn.distance(1)>scn.distance(end));
    restcode = zeros(length(caim.Df),numbin,max(scn.rounds));
    

    %% Read out running related events
    ...
    %% Read out resting related events
    
    for j = 1 : size(caim.S_bin,1)   
        for i = strtrnd:max(scn.rounds)
            disttemp = scn.distance(scn.running==0 & scn.rounds==i);
        %                catemp = caim.S_act(j,scn.running==1 & scn.rounds==i)/caim.Df_act(j);
            catemp = caim.S_bin(j,scn.running==0 & scn.rounds==i);
            if strtrnd == 0
                for k = 1:numbin
                    int = find(disttemp>(k-1)*spcbin+1 & disttemp<k*spcbin);  
                    if ~isempty(int)
                        restcode(j,k,i+1) = sum(catemp(int));
                    end
                end
            else
                for k = 1:numbin
                    int = find(disttemp>(k-1)*spcbin+1 & disttemp<k*spcbin);  
                    if ~isempty(int)
                        restcode(j,k,i+1) = mean(catemp(int));
                    end
                end
            end
        end
...
Vct = zeros(size(spcsort,1),1);
VctL = zeros(size(spcsort,1),1);
VctSpec = cell(size(spcsort,1),1);
...
%               d = [d;exp(Alpha(b(jj))*1j)];
              ...
              % angle weighted by the time spend in that bin (For Danielson
              % definition or the DG paper)
    ...
    % This defenition is used in the DG paper 
    Vct(i) = sum(dd)*sum(d)/length(d)^2; % place coding vector
    VctL(i) = abs(sum(dd)*sum(d)/length(d)^2); % place coding vector length
   
    % This defenition just calculates the mean of the angles 
%     spcpol{i,1} = sum(d)/length(d); % place coding vector
%     spcpol{i,2} = abs(sum(d)/length(d)); % place coding vector length

     VctSpec{i} = c; % alpha % rho % round % count in bin
...
spcpol(:,1) = num2cell(Vct);
spcpol(:,2) = num2cell(VctL);
spcpol(:,3) = VctSpec;
