function [plcVentral, plcDorsal,DNrestActFract, pooledf,r,numtrack] = PlaceFieldShiftPlot(CAIMcorr,cclustID,numses,DoPlot)
% this code calculates the place cell stabillity and how cells have shifted
% from one session to another 

% Further, the differences in event frequencies for different groups and
...
%     plccenter = abs(plccenter);
    ...
    % read out speed correlation
    rDN = cclust(isDeNovo,cclustID.speedcorr);
    rRO = cclust(isStable,cclustID.speedcorr);
    ...
    if j == 1
        ...
        r = 0;
        r(1,1,1:length(rRO)) = rRO;
        r(2,1,1:length(rDN)) = rDN; 
    else
        plcDorsal = plccenter;
        ...
        r(1,2,1:length(rRO)) = rRO;
        r(2,2,1:length(rDN)) = rDN; 
    end
    
    % read out place preference vectors   
end

numtrack = [a(1) a(2)-a(1) a(3)-a(2) a(4)-a(3) a(5) a(6)-a(5) a(7)-a(6) a(8)-a(7)];
...
%% plot comparissons within groups
if DoPlot == 1
    figure('color',[1 1 1],...
        'renderer','painters',...
        'visible','on',...
